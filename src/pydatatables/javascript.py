...
from .version import __version__ as pydatatables_version

_PYDATATABLES_UNDERSCORE_VERSION = (
    f"_pydatatables_{pydatatables_version.replace('.','_').replace('-','_')}"
)
_PYDATATABLES_READY_EVENT = f"pydatatables-{pydatatables_version}-ready"
...
def set_pydatatables_repr_html_methods(all_interactive: bool) -> None:
...
def init_notebook_mode(
    ...
    if GOOGLE_COLAB and not connected:
        warnings.warn(
            "The offline mode for pydatatables is not supposed to work in Google Colab. "
            "This is because HTML outputs in Google Colab are encapsulated in iframes."
        )
    _CONNECTED = connected

    set_pydatatables_repr_html_methods(all_interactive)
    ...
    if not connected:
        display(HTML(generate_init_offline_pydatatables_html(dt_bundle)))


def get_animated_logo(display_logo_when_loading: bool) -> str:
    """Return the HTML for the loading logo of PyDataTablesRenderers"""
    if not display_logo_when_loading:
        return ""
    return f"<a href=https://mwouts.github.io/itables/>{read_package_file('logo/loading.svg')}</a>"


def generate_init_offline_pydatatables_html(dt_bundle: Union[Path, str]) -> str:
    ...
    init_notebook_mode = replace_value(
        init_notebook_mode, "pydatatables-version-ready", _PYDATATABLES_READY_EVENT
    )
...
This is the <code>init_notebook_mode</code> cell from PyDataTablesRenderers v{pydatatables_version}<br>
...
def _table_header(
    ...
    if show_dtypes and (footer is False):
        ...
        formatted_dtypes = "".join(
            f"<th><small class='pydatatables-dtype'>{escape_html_chars(format_dtype(dt)) if escape_html else dt}</small></th>"
...
def to_html_datatable(
    ...
    **kwargs: Unpack[PyDataTablesOptions],
    ...
    check_itable_arguments(cast(dict[str, Any], dt_args), PyDataTablesRendererOptions)
...
def get_itable_arguments(
    ...
    **kwargs: Unpack[PyDataTablesOptions],
    ...
    """
    Return the arguments to be passed to the PyDataTablesRenderer class
    """
    if "import_jquery" in kwargs:
        raise TypeError(
            "The argument 'import_jquery' was removed in PyDataTablesRenderers v2.0. "
    ...
    dt_args = cast(PyDataTablesRendererOptions, kwargs)
...
def get_pydatatables_extension_arguments(
    df: Optional[DataFrameOrSeries],
    caption: Optional[str] = None,
    **kwargs: Unpack[PyDataTablesOptions],
) -> tuple[PyDataTablesRendererOptions, dict[str, Any]]:
    """
    This function returns two dictionaries that are JSON
    serializable and can be passed to the PyDataTablesRenderer extensions.
...
def check_table_id(
    ...
    kwargs: Union[PyDataTablesOptions, PyDataTablesRendererOptions],
    ...
    if table_id is None:
        ...
        return "pydatatables_" + str(uuid.uuid4()).replace("-", "_")
...
def set_default_options(
    kwargs: PyDataTablesOptions, *, use_to_html: bool, app_mode: bool
) -> None:
    if not app_mode:
    ...
    # The options for PyDataTablesRenderer in pydatatables will be checked later on
    check_itable_arguments(
        {
            k: v
            for k, v in kwargs.items()
            if k not in PyDataTablesRendererOptions.__optional_keys__
        },
        PyDataTablesOptions,
    )
...
def html_table_from_template(
    ...
    kwargs: PyDataTablesRendererOptions,
) -> str:
    if "css" in kwargs:
        raise TypeError(
  ...
  <tbody>
    <tr>
      <td style="vertical-align:middle; text-align:left">{get_animated_logo(display_logo_when_loading)}
        Loading pydatatables v{pydatatables_version} from {pydatatables_source}...
