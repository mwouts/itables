...
```{admonition} PyDataTables is the new name for ITables
:class: tip
`pydatatables` is the new name for ITables: same package, same functions, new
name. If you are using the historical `itables` package, it still works (it is
now a thin wrapper around `pydatatables`) and it keeps its
[documentation website](../index.html).
```

This website documents `pydatatables`, the DataTables renderer of the ITables
project. The ITables project also offers an [AG Grid](https://www.ag-grid.com/)
renderer, `pyaggrid`, which has its own
[documentation website](../pyaggrid/index.html).
