...
**Changed**
- We have sped up the CI: the `pytest` matrix jobs now install the wheel built once by `build-wheel` instead of each rebuilding the JS bundle from source, dependencies in that matrix are now installed with `uv` instead of `pip`, tests run in parallel with `pytest-xdist`, the conda package is now built with `rattler-build` instead of `conda-build`, and `pytest-pixi-updated` (which overlaps with the weekly scheduled run) is skipped on pull requests.
