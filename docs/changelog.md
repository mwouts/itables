...
**Added**
- The interactive HTML table now shows a plain, static HTML table by default, right next to the (initially hidden) interactive table, so that viewers that can't or won't run JavaScript (e.g. GitHub's static preview of `.ipynb` files, or an untrusted notebook) get a readable table instead of a broken or empty one. We have also added `itables.to_markdown_table()`, a simple, static Markdown table representation of a dataframe; `itables.show()` prints it instead of the interactive table when it can't expect a working, JavaScript-powered display at all (no IPython, or a non-Jupyter/terminal session). See the new [Fallbacks](fallbacks/static_preview.md) documentation section ([#575](https://github.com/mwouts/itables/issues/575))
...
**Fixed**
- The caption in the static HTML preview is now rendered as a table row (a `<tfoot>` row for the default `caption-side:bottom`, a leading `<thead>` row for `caption-side:top`) rather than a `<caption>` element. GitHub's notebook viewer strips `<caption>` tags, which left the caption text pushed above the table; the row survives sanitization and stays below (or above) the table, like in the interactive table ([#575](https://github.com/mwouts/itables/issues/575)). The "rows/columns not shown" note, if any, always comes in parentheses in that same, borderless row - joining the caption on its own line when there is one.
...
- Our ITables widget works in Panel. We have added an example Panel app to demonstrate this.
...
**Fixed**
- We have fixed a `ValueError` raised by `to_html_datatable` on columns with numpy floating scalars (`float32`, `float16`) containing `NaN` or `Inf` values ([#524](https://github.com/mwouts/itables/issues/524)). Thanks to [Luke Tsekouras](https://github.com/LukeGT) for reporting this!
...
**Added**
- We have added the FixedHeader extension of DataTables to the `dt_for_itables` package ([#496](https://github.com/mwouts/itables/pull/496)). Thanks to [Luke Tsekouras](https://github.com/LukeGT) for suggesting this!

**Changed**
- The HTML code generated by ITables is now deterministic. Thanks to [Tim Maier](https://github.com/TimOliverMaier) for suggesting and implementing this! ([#504](https://github.com/mwouts/itables/pull/504))
- We have introduced more line breaks in the HTML to make it easier to read ([#502](https://github.com/mwouts/itables/issues/502))
...
- We have added an optional `columnControl` to `option.py`. Thanks to [kultura-luke](https://github.com/kultura-luke) for pointing out at this ([#496](https://github.com/mwouts/itables/pull/496))
...
- We have updated the dependencies of the ITables extension for Dash to address three vulnerability issues ([#520](https://github.com/mwouts/itables/pull/520))
...
- Complex Polars objects are formatted using Polars' internal method `x._s.get_fmt` ([#471](https://github.com/mwouts/itables/issues/471))
...
**Fixed**
- The `typing_extensions` package is required for static type checking on Python <= 3.10 ([#473](https://github.com/mwouts/itables/pull/473))
