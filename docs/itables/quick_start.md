...
```{admonition} ITables is now called PyDataTables
:class: tip
`pydatatables` is the new name for ITables: same package, same functions, new name.
The `itables` package documented on this website still works - it is now a thin
wrapper around `pydatatables`. New projects should prefer
[`pydatatables`](pydatatables/index.html) directly. The ITables project also
offers an [AG Grid](https://www.ag-grid.com/) renderer: [`pyaggrid`](pyaggrid/index.html).
```
