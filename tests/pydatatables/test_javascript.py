...
from pydatatables.utils import UNPKG_DT_BUNDLE_CSS, UNPKG_DT_BUNDLE_URL


def test_get_itable_arguments():
    """
    As much as possible we want few arguments for the PyDataTablesRenderer class
    """
    try:
        import pandas as pd
    except ImportError:
...
def test_unpkg_links(url):
    ...
    response_no_version = requests.get("https://www.unpkg.com/pydatatables/")
...
def test_unpkg_urls_are_up_to_date():
    ...
    assert (
        UNPKG_DT_BUNDLE_URL
        == f"https://www.unpkg.com/pydatatables@{bundle_version}/dt_bundle.js"
    )
    assert (
        UNPKG_DT_BUNDLE_CSS
        == f"https://www.unpkg.com/pydatatables@{bundle_version}/dt_bundle.css"
    )
...
def test_show_dtypes_in_html():
    ...
    assert "<th><small class='pydatatables-dtype'></small></th>" in second_row
    assert "<th><small class='pydatatables-dtype'>f64</small></th>" in second_row
