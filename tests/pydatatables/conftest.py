...
try:
    dict_of_test_polars_dfs = pydatatables.sample_polars_dfs.get_dict_of_test_dfs()
except AttributeError:
...
def no_pydatatables_config_env_variable(monkeypatch):
    monkeypatch.delenv("PYDATATABLES_CONFIG", raising=False)
    yield
    monkeypatch.undo()
