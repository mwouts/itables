...
def test_float_formatting_polars():
    """Make sure that float formatting is the same for pandas and polars #430"""
    pytest.importorskip("polars")
    import polars as pl

    df = pl.DataFrame({"float": [math.pi]})
    with pl.Config(float_precision=6):
        assert datatables_rows(df) == "[[3.141593]]"
